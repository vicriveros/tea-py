<div>
    ...
    <x-content-section>
        ...
        <x-table>
            <x-slot name="botones">
                <a href="{{ route('profesional.create') }}" class="flex justify-center rounded bg-primary p-3 font-medium text-gray hover:bg-opacity-90 place-self-end"> Agregar </a>
            </x-slot>
            <thead>
                <tr>
            ...
            <tbody>
                @foreach ($personas as $persona)
                    <tr wire:key="{{ $persona->id }}">
                        ...
                        <x-td>
                            @can('editar medico')
                                <a href="{{ route('profesional.edit', $persona->id) }}"> 
